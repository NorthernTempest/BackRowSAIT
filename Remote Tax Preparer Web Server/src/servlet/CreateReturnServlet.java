...
/**
 * Servlet for creating a new tax return.
 *
...
public class CreateReturnServlet extends HttpServlet {
	...
	/**
	 * The earliest date a user can enter as their date of birth.
	 */
	private static final Calendar DOBCutOff = new GregorianCalendar(1900, 01, 01);

	/**
	 * a NewReturnForm object that represents the form filled out by a user creating
	 * a new return.
	 */
	...
	/**
	 * a helper method to validate all form inputs
	 * 
	 * @param request - the request containing form information to validate
	 * @throws Exception - contains error message to display when a form field is
	 *                   not valid
	 */
	private void validateForm(HttpServletRequest request) throws Exception {
		 ...
		 */
		if ((nrf.isCanadaPost() != true && nrf.isCRAOnline() != true && !nrf.isAlreadyRegistered())
				|| (nrf.isCRAOnline() == true && nrf.isAlreadyRegistered() == true)) {
	...
	/**
	 * A helper method to backfill the form if not validated.
	 * 
	 * @param request - the request containing form informtion to backfill
	 */
